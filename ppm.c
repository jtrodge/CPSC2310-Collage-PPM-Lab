...
void imageResize(header_t* old_h, pixel_t** original, double ratio, pixel_t** newImage) {
  int newWidth;
  int newHeight;
  // Shrinks the image into the appropriate ratio size
  double relativeWidth = old_h->width * ratio;
  double relativeHeight = old_h->height * ratio;
  for (int i = 0; i < relativeWidth; i++) {
    for (int j = 0; j < relativeHeight; j++) {
      // Shrinks image
      if (ratio < 1) {
        newWidth = (int)(i / ratio);
        newHeight = (int)(j / ratio);
      }
      // Enlarges image
      else {
        newWidth = (int)(i * ratio);
      ...
      newImage[i][j].r = original[newWidth][newHeight].r;
      newImage[i][j].g = original[newWidth][newHeight].g;
      newImage[i][j].b = original[newWidth][newHeight].b;
    }
  }
}

void collageImages(header_t* old_h, pixel_t** copy_pixel1, double ratio, pixel_t** copy_pixel2) {
  double relativeWidth = old_h->width * ratio *.5;
  double relativeHeight = old_h->height * ratio *.5;
  for (int i = 0; i < relativeWidth; i++) {
    for (int j = 0; j < relativeHeight; j++) {
      // Combines the image into 1
      copy_pixel2[i][j].r = copy_pixel1[i][j].r;
      copy_pixel2[i][j].g = copy_pixel1[i][j].g;
      copy_pixel2[i][j].b = copy_pixel1[i][j].b;
    }
  }
}
